...
V3에서는 실제 DUT `.scs` 안에서 `R`, `C`, `Q`, `M`, `D` primitive instance가 발견되면 netlist 기반 면적을 우선 사용합니다. 이 경우 `devices.csv`의 예제 `opamp`/`capacitor` 행은 면적에 더하지 않습니다. Verilog-A wrapper처럼 netlist primitive가 없을 때만 `devices.csv`를 fallback으로 사용합니다.
...
- `tb_tran_static.scs`: static power 측정용 DC-input transient testbench
- `tb_tran.scs`: active power/performance 측정용 transient simulation testbench
- `export.ocn`: PSF 결과를 CSV로 export하는 OCEAN script
- `ac.csv`: AC 결과
- `tran_static.csv`: static power 측정용 transient 결과
...
- `spectre_tran_static.log`: Spectre static transient log
...
- static run: `VIN`을 DC common-mode로 고정하고 transient를 돌린 뒤 steady 구간 평균 `mean(abs(VDD * IDD))`를 `power_static_w`로 사용
- active run: 기존 transient 입력, 예를 들어 sine/PWL 입력을 넣고 steady 구간 평균 `mean(abs(VDD * IDD))`를 `power_total_w`로 사용
- dynamic power: `power_dynamic_w = max(power_total_w - power_static_w, 0)`
- `devices.csv`의 `static_current_a` 기반 값은 `power_static_estimate_w`로만 기록하며, static run CSV가 없을 때 fallback으로 사용
...
이 방식은 한 번의 active transient에서 5th percentile로 static을 추정하는 방식보다 명확합니다. 같은 bias, 같은 load 조건에서 입력만 DC/active로 바꿔 비교하므로 static/dynamic 분리가 더 안정적입니다.
